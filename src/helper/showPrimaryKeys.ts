...
export async function showTablePrimaryKey(connection: Connection, tableName?: string): Promise<void> {
  if (tableName) {
    ...
    if (rows.length > 0) {
      ...
      rows.forEach((row: any, index: number) => {
        ...
        if (currentTable !== tableName) {
          if (currentTable !== '') {
            console.log(currentTable.padEnd(40) + pkColumns.join(', ').padEnd(40));
          }
